...
description: |
  **WORKFLOW SKILL** — Route MCP server requests for AWS knowledge, IaC,
  pricing, security, IAM, and CloudWatch. Apply selection rules, degraded-mode
  fallbacks per §3.5, and per-server timeout budgets per O2.
version: 0.1.0
---

## When to Use

Apply this skill any time an agent needs to consult an AWS MCP server. That includes the orchestrator's grounding step, every L3 specialist's MCP call, and any rule (§6) that delegates to a server. Do not apply this skill for non-AWS knowledge; use the agent's own reasoning or a non-MCP source.
...
- The agent is about to issue a tool call whose name might breach the §O3 tool-name budget; this skill verifies the resolved server key.
...
3. **Select the tool.** Consult the per-server tool catalogue in `references/server-roster.md`. Pick the narrowest tool that satisfies the request. Wide tools (e.g. broad documentation search) waste the per-specialist MCP-call budget per O5.
4. **Issue the call within the timeout budget.** The per-server `timeoutMs` value in `.mcp.json` is the per-call budget; the agent's MCP-call budget per O5 is the per-invocation cap. Both must be respected.
...
6. **Record the citation.** Every successful MCP call must be recorded in the grounding ledger via the `aws-grounding-cache` skill, producing a `<server>:<short-key>` reference for use in `grounded-by` fields per §9.5.
...
- **Do not silently drop signal on MCP failure.** Per §3.5 every degraded response must be labelled in the merged orchestrator output; fallback behaviour is documented per server.
- **Do not exceed the per-specialist MCP-call budget from O5.** Discovery 8, solution-architect 12, implementation 16. Exhaustion escalates back to the orchestrator with a `budget-exhausted` marker.
- **Do not mix logical names with `.mcp.json` keys.** Tool calls must use the short keys (`kb`, `iac`, `cost`, `sec`, `iam`, `cw`); the descriptive names from §3.1 are documentation references only.
...
- This skill MUST NOT add new MCP servers to `.mcp.json` at runtime; the v0.1.0 server set is fixed at six per §3.1.
- This skill MUST NOT call MCP servers in the v0.2 candidate list (§3.2) until they are wired with a version bump and an explicit ADR.
...
- The chosen tool would not breach the O3 tool-name budget unless it is in `tests/gates/cache/known-overshoots.txt`.
...
- The per-specialist MCP-call budget per O5 is not exceeded by issuing this call.
