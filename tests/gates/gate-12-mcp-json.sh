#!/usr/bin/env bash
# Gate 12: .mcp.json lists every server in §3.1 (by short key per O3),
# every entry carries a pinned `version` (per §16.2) and a `timeoutMs` (per O2).
# Per SPEC §11.A gate 12.
...
# Required server keys per the README/SPEC short-key mapping
...
[[ -z "$missing" && -z "$extra" ]] || gate_fail "$GATE" ".mcp.json server set does not match §3.1"
