...
for dir in "${skill_dirs[@]}"; do
  ...
  # Mandatory block-scalar prefix
  if [[ "$description" != "**WORKFLOW SKILL** —"* ]]; then
    gate_warn "$GATE" "$skill_md: description must begin with '**WORKFLOW SKILL** —'"
    failed=$((failed + 1))
  fi
...
gate_pass "$GATE" "${#skill_dirs[@]} skill descriptions match quality bar"
