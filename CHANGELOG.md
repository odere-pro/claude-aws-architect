...
- **MCP roster version bump (`feat(deps)`).** Pin the four drifted `awslabs.*` MCP servers in `.mcp.json` (both the `version` field and the matching `==X.Y.Z` in `args`) to their PyPI latest stable, surfaced by the nightly `mcp-version-skew` workflow: `iac` `1.0.17` → `1.0.19`, `cost` `1.0.28` → `1.0.30`, `iam` `1.0.18` → `1.0.20`, `cw` `0.0.26` → `0.1.4`. `kb` (http) and `sec` (`0.1.7`, already latest) are unchanged. Also harden `mcp-version-skew.yml`: a new "Ensure tracking labels exist" step idempotently creates the `mcp-version-skew` and `dependencies` labels before the issue-management steps, fixing the `could not add label: 'mcp-version-skew' not found` crash that failed the scheduled run on a label-less repo.
