...
_(Pending PRs 25–26.)_ Hooks registry at `hooks/hooks.json` with six scripts under `hooks/scripts/`. Default-enabled: `aws-secret-scanner`, `aws-api-write-guard`. Opt-in via `.claude/claude-aws-architect.local.md`. Hook-script contract enforced by gate 11.
